...
---------------------------------------
Supports: FLUX, Stable Diffusion (1.x/2.x/3.x), SANA, Lumina, Lumina2, Janus,
...
Model is selected via config.yaml:
    model_name: "flux"          # flux | sd | sd3 | sd3_onlb | sd3_peakback | sd3_ugile |
                                 # sana | sana_ugile | lumina | lumina2_ugile | janus |
                                 # cogvideo | mini_gemini

Multi-seed (sd3_onlb, sd3_peakback, sd3_ugile, sana_ugile, lumina2_ugile only):
    ...
    (sd3_peakback / sd3_ugile / sana_ugile / lumina2_ugile, see the matching
    config block).
    Per-seed/branch diagnostics are reported at the end.

    sana_ugile is the SANA port of sd3_ugile (see latent_escape_sampler_sana.py
    + pipeline_wrapper_sana.py) — same UGILE algorithm, SANA backbone. Use
    config_sana.yaml as a ready-made starting config for it.

    lumina2_ugile is the Lumina-Image-2.0 port of sd3_ugile (see
    latent_escape_sampler_lumina2.py + pipeline_wrapper_lumina2.py) — same
    UGILE algorithm, Lumina2 backbone (reversed timestep convention +
    transformer-output sign flip handled internally). Use config_lumina2.yaml
    as a ready-made starting config for it.
"""
...
# PeakBack import — graceful fallback if file not yet on path
try:
    from peakback_sampler import run_sd3_peakback as _run_sd3_peakback
    _PEAKBACK_AVAILABLE = True
except ImportError:
...
# UGILE (SANA port) import — graceful fallback if file not yet on path
try:
    from latent_escape_sampler_sana import run_sana_ugile as _run_sana_ugile
    _UGILE_SANA_AVAILABLE = True
except ImportError:
    _UGILE_SANA_AVAILABLE = False

# UGILE (Lumina2 port) import — graceful fallback if file not yet on path
try:
    from latent_escape_sampler_lumina2 import run_lumina2_ugile as _run_lumina2_ugile
    _UGILE_LUMINA2_AVAILABLE = True
except ImportError:
    _UGILE_LUMINA2_AVAILABLE = False

# UGILE (PixArt-Sigma port) import — graceful fallback if file not yet on path
try:
    from latent_escape_sampler_pixart_sigma import run_pixart_sigma_ugile as _run_pixart_sigma_ugile
    _UGILE_PIXART_SIGMA_AVAILABLE = True
except ImportError:
    _UGILE_PIXART_SIGMA_AVAILABLE = False

# UGILE (SD2.1 port) import — graceful fallback if file not yet on path
try:
    from latent_escape_sampler_sd21 import run_sd21_ugile as _run_sd21_ugile
    _UGILE_SD21_AVAILABLE = True
except ImportError:
    _UGILE_SD21_AVAILABLE = False
...
def parse_args():
    ...
    p.add_argument("--prompt-offset", type=int, default=None,
                   help="Absolute index of this process's first prompt within "
                        ...
                        "0 on every process.")
    p.add_argument("--ugile-enabled", type=str, choices=["true", "false"], default=None)
    return p.parse_args()
...
def resolve_args(args, cfg: dict) -> dict:
    ...
    if args.ugile_enabled is not None:
        cfg["ugile"]["enabled"] = args.ugile_enabled.lower() == "true"
    ...
    return {
        ...
        "ugile_enabled":   cfg.get("ugile", {}).get("enabled", False),
...
def main():
    ...
    # Register PeakBack runner if available
    ...
    # Register UGILE (SANA port) runner if available
    if _UGILE_SANA_AVAILABLE:
        MODEL_REGISTRY["sana_ugile"] = _run_sana_ugile

    # Register UGILE (Lumina2 port) runner if available
    if _UGILE_LUMINA2_AVAILABLE:
        MODEL_REGISTRY["lumina2_ugile"] = _run_lumina2_ugile

    # Register UGILE (PixArt-Sigma port) runner if available
    if _UGILE_PIXART_SIGMA_AVAILABLE:
        MODEL_REGISTRY["pixart_sigma_ugile"] = _run_pixart_sigma_ugile

    # Register UGILE (SD2.1 port) runner if available
    if _UGILE_SD21_AVAILABLE:
        MODEL_REGISTRY["sd21_ugile"] = _run_sd21_ugile
    ...
    if model_name in ("sd3_onlb", "sd3_peakback", "sd3_ugile", "sana_ugile", "lumina2_ugile", "pixart_sigma_ugile", "sd21_ugile"):
        seeds = opts["seeds"]
