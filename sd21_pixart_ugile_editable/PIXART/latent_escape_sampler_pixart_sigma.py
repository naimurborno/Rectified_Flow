...
class PixArtSigmaUGILESampler:
    """Threshold-based UGILE sampler for PixArtSigma."""

    def __init__(self, transformer, scheduler, cfg: dict, wrapper=None, device: str = "cuda", eps: float = 1e-8, num_grad_steps: int = 50, sigma_lo: float = -1.0, sigma_hi: float = 1.0, escape_scale: float = 1.0, theta_max: float = 2.0, walk_steps: int = 10, branch_noise: float = 0.7, J: int = 1, noise_scale: float = 15.0, gamma: float = 1.2):
        ...
        self.num_steps = flow_cfg.get("num_steps", 20)
        self.guidance_scale = flow_cfg.get("guidance_scale", 4.5)
    ...
    def _sigma_at(self, k, timestep, n_steps):
        if not hasattr(self.scheduler, "sigmas"):
            raise RuntimeError("PixArtSigma scheduler does not expose sigmas required for common x0 discrepancy")
        return max(float(self.scheduler.sigmas[k].item()), 1e-4)
    ...
    def _model_forward(self, x, timestep, text_embeddings, extra_conditioning, return_split=False):
        ...
        t_batch = timestep.expand(latent_input.shape[0]).to(device=device)
        kwargs = dict(hidden_states=latent_input, timestep=t_batch, encoder_hidden_states=text_embeddings, return_dict=False)
...
def run_pixart_sigma_ugile(opts: dict):
    ...
    sampler = PixArtSigmaUGILESampler(transformer=wrapper.transformer, scheduler=wrapper.scheduler, cfg=cfg, wrapper=wrapper, device=device)
    ...
    for local_idx, prompt in enumerate(prompts):
        ...
        for seed in seeds:
            ...
            print(f"[UGILE-PixArtSigma]   ugile_enabled={ugile_enabled} original_latent_hash={original_hash}")
            ...
            if ugile_enabled:
                ...
                print(f"[UGILE-PixArtSigma]   Branch 0 -> {out_path}")
    ...
    print(f"\n[UGILE-PixArtSigma] Done — {len(records)} manifest record(s)")
    return records
