...
class PixArtSigmaPipelineWrapper:
    ...
    def load(self):
        ...
        components_model_id = self.cfg.get(
            "pipeline_components_model_id",
        ...
        try:
            ...
            if "model_index.json" not in str(exc):
                raise
            print(
                "[Pipeline] Full pipeline metadata was not found in the "
                "PixArt-Sigma checkpoint repo; loading shared T5/VAE "
                f"components from {components_model_id} and transformer "
                f"from {model_id}."
            )
        ...
        print(f"[Pipeline] model_id={model_id}")
        print(f"[Pipeline] components_model_id={components_model_id}")
        print(f"[Pipeline] pipeline_class={self.pipe.__class__.__name__}")
        print(f"[Pipeline] max_sequence_length={self.cfg.get('max_sequence_length', 300)}")
        print(f"[Pipeline] clean_caption={self.cfg.get('clean_caption', True)}")
        print(f"[Pipeline] ugile_enabled={self.cfg.get('ugile', {}).get('enabled', False)}")
        ...
        try:
            ...
            raise RuntimeError(
                "Failed to construct the intended PixArt-Sigma DPMSolverMultistep scheduler"
            ) from exc
    ...
    def encode_prompt(self, prompt: str, negative_prompt: str = ""):
        ...
        max_seq_len = self.cfg.get("max_sequence_length", 300)
        clean_caption = self.cfg.get("clean_caption", True)
    ...
    def generate(self, prompt: str, negative_prompt: str = "", seed: int = 42) -> Image.Image:
        ...
        result = self.pipe(
            ...
            max_sequence_length = self.cfg.get("max_sequence_length", 300),
            clean_caption       = self.cfg.get("clean_caption", True),
            generator           = generator,
        )
